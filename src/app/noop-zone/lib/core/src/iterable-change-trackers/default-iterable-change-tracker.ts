...
import { IterableChangeObserver, IterableChangeTracker, IterableChangeTrackerFactory } from "./iterable-change-trackers";

const trackByIdentity = (index: number, item: any) => item;

export class DefaultIterableChangeTracker<V> implements IterableChangeTracker<V> {
  ...
  constructor(trackByFn?: TrackByFunction<V> | null | undefined) {
  ...
  applyChanges(observer: IterableChangeObserver<V>): void {
    ...
    while (nextIt || nextRemove) {
      ...
      const record: IterableChangeRecord_<V> = !nextRemove ||
              nextIt &&
                  nextIt.currentIndex! <
  ...
  findChanges(collection: NgIterable<V> | null | undefined): boolean {
    if (collection == null) collection = [];
    if (!isListLikeIterable(collection)) {
      throw  new Error(`Error trying to diff '${
                  JSON.stringify(collection)}'. Only arrays and iterables are allowed`);
    }
    ...
    if (Array.isArray(collection)) {
      ...
      let iteratorResult: IteratorResult<V, any>

      while (!(iteratorResult = iterator.next()).done) {
        if (record === null || !Object.is(record.trackById, itemTrackBy)) {
          record = this._mismatch(record, iteratorResult.value, itemTrackBy, index);
          mayBeDirty = true;
        } else {
          if (mayBeDirty) {
            // TODO(misko): can we limit this to duplicates only?
            record = this._verifyReinsertion(record, iteratorResult.value, itemTrackBy, index);
          }
          if (!Object.is(record.item, iteratorResult)) this._addIdentityChange(record, iteratorResult.value);
        }
        record = record._next;
        index++;
      }
   ...
   */
  private _mismatch(record: IterableChangeRecord_<V>|null, item: V, itemTrackBy: any, index: number):
   ...
   *
   */
  private _verifyReinsertion(record: IterableChangeRecord_<V>, item: V, itemTrackBy: any, index: number):
      IterableChangeRecord_<V> {
   ...
   */
  private _truncate(record: IterableChangeRecord_<V>|null) {
  ...
  private _addAfter(
    ...
    if (this._additionsTail === null) {
      ...
      // record._prevAdded = null;
    } else {
      // record._prevAdded = this._additionsTail;
  ...
  private _remove(record: IterableChangeRecord_<V>): IterableChangeRecord_<V> {
    return this._addToRemovals(this._unlink(record));
  }

  private _unlink(record: IterableChangeRecord_<V>): IterableChangeRecord_<V> {
    if (this._linkedRecords !== null) {
  ...
  private _addToMoves(record: IterableChangeRecord_<V>, toIndex: number): IterableChangeRecord_<V> {
    ...
    if (this._movesTail === null) {
      ...
      // record._prevMoved = null;
    } else {
      // record._prevMoved = this._movesTail;
  ...
  private _addIdentityChange(record: IterableChangeRecord_<V>, item: V) {
    record.item = item;
    if (this._identityChangesTail === null) {
      this._identityChangesTail = this._identityChangesHead = record;
      record._prevIdentityChange = null;
    } else {
      record._prevIdentityChange = this._identityChangesHead;
...
class IterableChangeRecord_<V> implements IterableChangeRecord<V> {
  ...
  /** @internal */
  _prevIdentityChange: IterableChangeRecord_<V>|null = null;
...
export class DefaultIterableChangeTrackerFactory implements IterableChangeTrackerFactory {
  supports(objects: any): boolean {
    return isListLikeIterable(objects);
  }
  create<V>(trackByFn: TrackByFunction<V> | null | undefined): IterableChangeTracker<V> {
    return new DefaultIterableChangeTracker(trackByFn);
  }
