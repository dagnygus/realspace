...
import { NzIfDirecive, NzIfModule, provideNzIfConfiguration } from "../../template";
...
describe('[NzIf] optimized. render notificarions.', () => {

  describe('Raw walues with waitUntilSchedulingDone() function.', () => {

  setupEnviroment();

    it('Should not run callbacks.', async () => {
  ...
  describe('Promises using flushScheduler()', () => {
    setupEnviroment()
    it('SShould not run callbacks.', fakeAsync(() => {
      const component = createTestComponent();
      expect(component.directives).toBeFalsy();
