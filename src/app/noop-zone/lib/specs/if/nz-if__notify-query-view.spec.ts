...
import { NZ_QUERY_VIEW, NzIfModule, QueryView } from "../../template";
...
class TestOneDirective implements DoCheck {
  enable = false;
  init = false;

  checkCount = 0;

  constructor(component: TestOneComponent,
              @Inject(NZ_QUERY_VIEW) queryView: QueryView) {
    component.testOneDirective = this;
...
class TestOneComponent {
  ...
  testOneDirective: TestOneDirective | null = null;
  ...
  reset(): void {
    if (this.testOneDirective) {
      this.testOneDirective.checkCount = 0;
    }
  }

  enableListening(): void {
    if (this.testOneDirective) {
      this.testOneDirective.enable = true;
    }
  }
  ...
  getCheckCount() {
    return this.testOneDirective?.checkCount;
  }
}
...
function createTestOneComponent(): TestOneComponent {
  return TestBed.createComponent(TestOneComponent).componentInstance;
}
...
function setupEnviroment(): void {
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      ...
      declarations: [ TestOneComponent, TestOneDirective ],
...
describe('[nzIf] notifying query view', () => {
      ...
      describe('Raw values', () => {
        describe('using waitUntilSchedulingDone()', () => {
          ...
          it('Query view should run change detection',async () => {
            const component = createTestOneComponent();
