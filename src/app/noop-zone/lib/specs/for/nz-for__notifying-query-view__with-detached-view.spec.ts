...
@Component({
  ...
  template: `
    <ng-container *nzQueryView="queryViewPriority">
      <ng-container *nzDetachedView="detachedViewPriority">
        <div testDir></div>
...
class TestComponent {
  ...
  detachedViewPriority = Math.round(4 * Math.random()) + 1;
...
function setupEnviroment(): void {
  beforeEach(async () => {
    ...
    await TestBed.configureTestingModule({
      imports: [ NzForDirective, NzQueryViewModule, NzDetachedViewModule ],
...
for (let prio = 1; prio < 6; prio++) {
  describe(`[NzFor] with detached view: Notifying query view. Priority ${prio}.`, () => {
    ...
    describe('Raw value. Using flushScheduler() function.', () => {
      setupEnviroment();
      it('Should increase check count.', () => {
        const component = createComponent();
        component.items = getItems();
        component.priority = prio;
        flushScheduler();
    ...
    describe('Promise. Using waitUntilSchedulingDone() function.', () => {
      ...
      it('Should increase check count.', async () => {
        ...
        promise = new Promise<Item[]>((resolve) => {
          setTimeout(() => { resolve(getChangedItems(items)); });
        });
